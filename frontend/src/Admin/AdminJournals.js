...
const AdminJournals = () => {
  ...
  const handleApprove = async (journalId) => {
    try {
      const { data } = await axios.post(
        "http://localhost:8080/journal/approve",
        { journalId: journalId }
      );
  ...
  const handleDel = async (journalID) => {
    try {
      const { data } = await axios.post(
        "http://localhost:8080/journal/delete",
        { journalID: journalID }
      );
  ...
  const FacultyCard = ({ faculty }) => {
    return (
      ...
      >
        ...
        <p className="text-gray-700 mb-1">
          <strong>Total Journals:</strong> {faculty.journals.length}
        </p>
      </div>
    );
  };
            ...
            {faculty.journals.map((journal) => (
              ...
              >
                <div className="text-gray-700 flex justify-center items-start gap-4 flex-col">
                  <p>
                    <strong>Name:</strong> {journal.name}
                  </p>
                  <p>
      ...
      <div className="flex">
        ...
        <div className="bg-[url('https://img.freepik.com/free-vector/teacher-collection-concept_52683-37015.jpg')] bg-cover h-screen w-full">
          <div className="w-full p-6 h-full backdrop-blur-sm overflow-auto">
            <h2 className="text-3xl font-bold mb-4">Approve Journals</h2>
