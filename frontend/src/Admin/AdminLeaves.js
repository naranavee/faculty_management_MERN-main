...
const AdminLeaves = () => {
  ...
  const handleApprove = async (leaveId) => {
    try {
      const { data } = await axios.post(
        "http://localhost:8080/leaves/approve",
        { leaveId: leaveId }
      );
  ...
  const handleDel = async (leaveId) => {
    try {
      await axios.post("http://localhost:8080/leaves/delete", {
        leaveId: leaveId,
      });
      toast.success("Leaves deleted successfully");
      getAllFaculties(); // Refresh faculty list after deletion
    } catch (error) {
  ...
  const FacultyCard = ({ faculty }) => {
    return (
      ...
      >
        ...
        <p className="text-gray-700 mb-1">
          <strong>Total Leaves:</strong> {faculty.leaves.length}
        </p>
      </div>
    );
  };
  ...
  const Modal = ({ faculty, onClose }) => {
    ...
    return (
      <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
        <div className="bg-white p-6 rounded-lg shadow-lg w-[600px] max-h-[600px] overflow-auto">
          <div className="flex justify-between items-center">
            <h3 className="text-2xl font-semibold mb-4">
              Leaves Requested By {faculty.name}
            </h3>
            <button
              onClick={onClose}
            ...
            {faculty.leaves.map((leave) => (
              ...
              >
                <div className="text-gray-700 flex justify-center items-start gap-4 flex-col">
                  ...
                  <p>
                    <strong>Approved:</strong> {leave.approved ? "Yes" : "No"}
                  </p>
                </div>
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={() => handleDel(leave._id)}
                    className="bg-red-400 text-white p-2 rounded hover:bg-red-600 transition "
                  >
                    Delete
                  </button>
                </div>
      ...
      <div className="flex">
        ...
        <div className="bg-[url('https://img.freepik.com/free-vector/teacher-collection-concept_52683-37015.jpg')] bg-cover h-screen w-full">
          <div className="w-full p-6 h-full backdrop-blur-sm overflow-auto">
            <h2 className="text-3xl font-bold mb-4">Approve Leaves</h2>
